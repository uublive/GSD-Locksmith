#!/usr/bin/env bash
# tests/test-validate.sh — Fixture-based test harness for hooks/lib/validate.sh
# Usage: bash tests/test-validate.sh [gap|dup|drift|stale]
...
VALIDATE_SH="$REPO_ROOT/hooks/lib/validate.sh"

if [[ ! -f "$VALIDATE_SH" ]]; then
  echo "ERROR: hooks/lib/validate.sh not found (expected at $VALIDATE_SH)" >&2
  exit 1
fi
